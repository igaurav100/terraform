...
resource "azurerm_mysql_server" "datauk_db_server" {
  ...
  version                           = "8.0"
  auto_grow_enabled                 = var.db_auto_grow_enable
...
# Create Database
resource "azurerm_mysql_database" "datauk_db" {
  name                = "webappdb1"
  resource_group_name = azurerm_resource_group.rg-uks-ai-dev.name
  server_name         = azurerm_mysql_server.datauk_db_server.name
  charset             = "utf8"
